...
export class SelfServiceController {
  ...
  async createChangeRequest(
    @Req() req: any,
    @Body() dto: CreateChangeRequestHttpDto,
  ) {
    const employeeId = await this.getEmployeeId(req);
  ...
  async getDocumentDownloadUrl(
    ...
    const employeeId = await this.getEmployeeId(req);
    return {
      url: await this.profileService.getDocumentDownloadUrl(
        employeeId,
        documentId,
      ),
    };
  ...
  async flagDiscrepancy(@Req() req: any, @Body() dto: FlagDiscrepancyHttpDto) {
    const employeeId = await this.getEmployeeId(req);
    return this.attendanceService.flagDiscrepancy(employeeId, {
  ...
  async downloadPayslip(
    ...
    const employeeId = await this.getEmployeeId(req);
    return {
      url: await this.payslipService.downloadPayslip(employeeId, payrollId),
    };
  }
  ...
  async downloadBuktiPotong(@Req() req: any, @Query('year') year: string) {
    ...
    const y = parseInt(year, 10) || new Date().getFullYear();
    return {
      url: await this.payslipService.downloadBuktiPotong(employeeId, y),
    };
  }
  ...
  async createOvertimeRequest(
    @Req() req: any,
    @Body() dto: CreateOvertimeRequestHttpDto,
  ) {
    const employeeId = await this.getEmployeeId(req);
