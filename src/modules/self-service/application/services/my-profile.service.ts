...
const WHITELISTED_FIELDS = ['phone', 'address', 'emergencyContact']
...
export class MyProfileService implements MyProfileServicePort {
  ...
  async createChangeRequest(
    employeeId: string,
    data: { fieldName: string; oldValue: string; newValue: string; reason: string },
  ): Promise<any> {
  ...
  async getDocuments(employeeId: string): Promise<any[]> {
    const docRepo = this.dataSource.getRepository(EmployeeDocumentTypeOrmEntity)
    return docRepo.find({
      where: { employeeId },
  ...
  async getDocumentDownloadUrl(employeeId: string, documentId: string): Promise<string> {
    const docRepo = this.dataSource.getRepository(EmployeeDocumentTypeOrmEntity)
