...
describe('MyProfileService', () => {
  ...
  beforeEach(async () => {
    mockDataSource.getRepository.mockImplementation((entity: any) => {
      ...
      if (entity.name === 'EmployeeDocumentTypeOrmEntity')
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        { provide: DataSource, useValue: mockDataSource },
        {
          ...
          useValue: mockProfileChangeRequestRepo,
        },
      ],
    }).compile();
  ...
  describe('getEmployeeIdFromUserId', () => {
    ...
    it('should throw NotFoundException if no employee linked', async () => {
      ...
      await expect(service.getEmployeeIdFromUserId('user-1')).rejects.toThrow(
        NotFoundException,
      );
    });
  ...
  describe('updateProfile', () => {
    it('should update whitelisted fields', async () => {
      ...
      const result = await service.updateProfile('emp-1', {
        phone: '0987654321',
        address: 'New Address',
      });
  ...
  describe('getDocumentDownloadUrl', () => {
    ...
    it('should throw NotFoundException if document not found', async () => {
      ...
      await expect(
        service.getDocumentDownloadUrl('emp-1', 'non-existent'),
      ).rejects.toThrow(NotFoundException);
    });
  });
});
