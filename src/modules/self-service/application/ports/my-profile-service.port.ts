...
export interface MyProfileServicePort {
  ...
  createChangeRequest(employeeId: string, data: { fieldName: string; oldValue: string; newValue: string; reason: string }): Promise<any>
  getDocuments(employeeId: string): Promise<any[]>
  getDocumentDownloadUrl(employeeId: string, documentId: string): Promise<string>
}
