...
describe('EmployeeService', () => {
  ...
  describe('findAll', () => {
    it('should return paginated employees', async () => {
      ...
      expect(employeeRepo.findAll).toHaveBeenCalledWith({ search: 'John', page: 1, limit: 10 })
    })
  ...
  describe('create', () => {
    it('should create employee with auto-generated number', async () => {
      const command = new CreateEmployeeCommand('John', 'Doe', 'john@test.com', '08123', 'fulltime', 'dept-1', 'pos-1', 5000000, '2024-01-15')
      const createdEmployee = { id: 'emp-1', employeeNumber: 'EMP-2026-0001', fullName: 'John Doe' }
    ...
    it('should throw NotFoundException when department not found', async () => {
      const command = new CreateEmployeeCommand('John', 'Doe', undefined, undefined, undefined, 'invalid-dept')
    ...
    it('should throw NotFoundException when position not found', async () => {
      const command = new CreateEmployeeCommand('John', 'Doe', undefined, undefined, undefined, 'dept-1', 'invalid-pos')
  ...
  describe('update', () => {
    it('should update employee fields', async () => {
      ...
      const command = new UpdateEmployeeCommand('Jane', 'Doe', 'jane@test.com', '08999')
      ...
      expect(employeeRepo.update).toHaveBeenCalledWith('1', expect.objectContaining({
        fullName: 'Jane Doe',
        email: 'jane@test.com',
        phone: '08999',
      }))
    ...
    it('should throw NotFoundException when employee not found', async () => {
      ...
      await expect(service.update('nonexistent', new UpdateEmployeeCommand())).rejects.toThrow(NotFoundException)
    })

    it('should validate department when updating departmentId', async () => {
      const existing = { id: '1', fullName: 'John Doe' }
      const command = new UpdateEmployeeCommand(undefined, undefined, undefined, undefined, undefined, 'invalid-dept')

      employeeRepo.findById.mockResolvedValue(existing)
      departmentRepo.findById.mockResolvedValue(null)

      await expect(service.update('1', command)).rejects.toThrow(NotFoundException)
    })

    it('should validate position when updating positionId', async () => {
      const existing = { id: '1', fullName: 'John Doe' }
      const command = new UpdateEmployeeCommand(undefined, undefined, undefined, undefined, undefined, undefined, 'invalid-pos')
    ...
    it('should auto-create user account when email is changed', async () => {
      const existing = { id: '1', fullName: 'John Doe', email: 'old@test.com' }
      const command = new UpdateEmployeeCommand(undefined, undefined, 'new@test.com')
    ...
    it('should not create user when email is unchanged', async () => {
      const existing = { id: '1', fullName: 'John Doe', email: 'same@test.com' }
      const command = new UpdateEmployeeCommand(undefined, undefined, 'same@test.com')
    ...
    it('should map baseSalary to basicSalary and hireDate to joinDate', async () => {
      const existing = { id: '1', fullName: 'John Doe' }
      const command = new UpdateEmployeeCommand(undefined, undefined, undefined, undefined, undefined, undefined, undefined, 8000000, '2024-06-01')
  ...
  describe('uploadDocument', () => {
    it('should create document for existing employee', async () => {
      ...
      expect(employeeRepo.createDocument).toHaveBeenCalledWith(expect.objectContaining({
        employeeId: '1',
        type: 'KTP',
        fileName: 'ktp.jpg',
        filePath: '/docs/ktp.jpg',
    ...
    it('should throw NotFoundException when employee not found', async () => {
      ...
      await expect(service.uploadDocument('nonexistent', { type: 'KTP', fileName: 'x', filePath: 'y' })).rejects.toThrow(NotFoundException)
    })
  })
  ...
  describe('addHistoryEvent', () => {
    ...
    it('should throw NotFoundException when employee not found', async () => {
      ...
      await expect(service.addHistoryEvent('nonexistent', {
        eventType: 'test',
        description: 'test',
        effectiveDate: '2024-01-01',
