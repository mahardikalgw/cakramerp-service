...
describe('PayrollEngineService', () => {
  ...
  describe('getPayrollRuns', () => {
    it('should return paginated payroll runs', async () => {
      const expected = {
        data: [{ id: 'run-1', month: 3, year: 2024 }],
        total: 1,
      };
      ...
      expect(payrollRepo.findAllRuns).toHaveBeenCalledWith({
        year: 2024,
        status: 'draft',
      });
    });
  ...
  describe('getPayrollRun', () => {
    ...
    it('should throw BadRequestException when run not found', async () => {
      ...
      await expect(service.getPayrollRun('nonexistent')).rejects.toThrow(
        BadRequestException,
      );
    });
  });
  ...
  describe('runPayroll', () => {
    it('should create payroll run and calculate for all active employees', async () => {
      ...
      expect(payrollRepo.createRun).toHaveBeenCalledWith(
      ...
      expect(payrollRepo.updateRun).toHaveBeenCalledWith(
        'run-1',
        expect.objectContaining({
    ...
    it('should throw BadRequestException if period already confirmed', async () => {
      payrollRepo.findRunByMonthYear.mockResolvedValue({
        id: 'run-1',
        status: 'confirmed',
      });

      await expect(service.runPayroll(3, 2024)).rejects.toThrow(
        BadRequestException,
      );
    });
    ...
    it('should delete existing draft and re-run', async () => {
      ...
      employeeRepo.findActiveEmployees.mockResolvedValue(employees);
      payrollRepo.createRun.mockResolvedValue({
        id: 'run-new',
        month: 3,
        year: 2024,
      });
    ...
    it('should include overtime pay in gross calculation', async () => {
      const employees = [
        { id: 'emp-1', fullName: 'John', basicSalary: 17300000 },
      ];
      ...
      expect(payrollRepo.createDetail).toHaveBeenCalledWith(
        expect.objectContaining({
  ...
  describe('confirmPayroll', () => {
    ...
    it('should throw BadRequestException when run not found', async () => {
      ...
      await expect(
        service.confirmPayroll('nonexistent', 'user-1'),
      ).rejects.toThrow(BadRequestException);
    });

    it('should throw BadRequestException when run is not draft', async () => {
      payrollRepo.findRunById.mockResolvedValue({
        id: 'run-1',
        status: 'confirmed',
      });

      await expect(service.confirmPayroll('run-1', 'user-1')).rejects.toThrow(
        BadRequestException,
      );
    });
  });
  ...
  describe('postToGL', () => {
    ...
    it('should throw BadRequestException when run not found', async () => {
      ...
      await expect(service.postToGL('nonexistent', 'user-1')).rejects.toThrow(
        BadRequestException,
      );
    });
    ...
    it('should throw BadRequestException when run is not confirmed', async () => {
      ...
      await expect(service.postToGL('run-1', 'user-1')).rejects.toThrow(
        BadRequestException,
      );
    });
  });
