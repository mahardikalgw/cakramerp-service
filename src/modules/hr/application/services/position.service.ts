...
export class PositionService implements PositionServicePort {
  ...
  async create(command: CreatePositionCommand): Promise<any> {
    ...
    const existing = await this.positionRepo.findByNameAndDepartment(command.name, command.departmentId)
    if (existing) throw new ConflictException('Position already exists in this department')
  ...
  async update(id: string, command: UpdatePositionCommand): Promise<any> {
    ...
    if (command.name || command.departmentId) {
      ...
      const existing = await this.positionRepo.findByNameAndDepartment(checkName, checkDeptId)
