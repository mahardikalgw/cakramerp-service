...
describe('ThrService', () => {
  ...
  describe('calculate', () => {
    it('should calculate THR for employees with >= 12 months service (full amount)', async () => {
      const employees = [
        { id: 'emp-1', fullName: 'John', joinDate: '2023-01-01', basicSalary: 5000000, employmentType: 'fulltime' },
      ]
      ...
      expect(thrRepo.create).toHaveBeenCalledWith(expect.objectContaining({
        employeeId: 'emp-1',
        employeeName: 'John',
        year: 2024,
    ...
    it('should pro-rate THR for employees with < 12 months service', async () => {
      const employees = [
        { id: 'emp-1', fullName: 'John', joinDate: '2023-10-01', basicSalary: 6000000, employmentType: 'fulltime' },
      ]
    ...
    it('should exclude employees with < 1 month of service', async () => {
      const employees = [
        { id: 'emp-1', fullName: 'New Guy', joinDate: '2024-03-15', basicSalary: 5000000, employmentType: 'fulltime' },
      ]
    ...
    it('should handle multiple employees with mixed eligibility', async () => {
      const employees = [
        { id: 'emp-1', fullName: 'Senior', joinDate: '2022-01-01', basicSalary: 10000000, employmentType: 'fulltime' },
        { id: 'emp-2', fullName: 'New', joinDate: '2024-12-15', basicSalary: 4000000, employmentType: 'contract' },
        { id: 'emp-3', fullName: 'Mid', joinDate: '2023-09-01', basicSalary: 7000000, employmentType: 'fulltime' },
      ]
  ...
  describe('confirm', () => {
    it('should confirm a calculated THR record and create journal entry', async () => {
      ...
      expect(thrRepo.update).toHaveBeenCalledWith('thr-1', expect.objectContaining({
        status: 'confirmed',
      }))
    ...
    it('should throw BadRequestException when record not found', async () => {
      ...
      await expect(service.confirm('nonexistent', 'user-1')).rejects.toThrow(BadRequestException)
    })
    ...
    it('should throw BadRequestException when record is not in calculated status', async () => {
      ...
      await expect(service.confirm('thr-1', 'user-1')).rejects.toThrow(BadRequestException)
    })
  })
})
