...
export class PayrollEngineService implements PayrollServicePort {
  ...
  async runPayroll(month: number, year: number): Promise<any> {
    ...
    if (
      existing &&
      (existing.status === 'confirmed' || existing.status === 'posted')
    ) {
    ...
    for (const emp of employees) {
      const detail = await this.calculateEmployeePayroll(
        payrollRun.id,
        emp,
        month,
        year,
  ...
  async postToGL(id: string, userId: string): Promise<any> {
    ...
    if (run.status !== 'confirmed') {
      throw new BadRequestException(
        'Only confirmed payroll runs can be posted to GL',
      );
    }
  ...
  private async calculateEmployeePayroll(
    ...
    const overtimeHours = await this.attendanceRepo.getOvertimeHours(
      employee.id,
      month,
      year,
    );
    ...
    const monthlyBpjsEmployee =
      bpjsKesehatanEmployee + bpjsJhtEmployee + bpjsJpEmployee;
    const annualTaxableIncome =
      (grossPay - monthlyBpjsEmployee) * 12 - this.PTKP;
