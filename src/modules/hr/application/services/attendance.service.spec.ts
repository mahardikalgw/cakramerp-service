...
describe('AttendanceService', () => {
  ...
  describe('getMonthlyGrid', () => {
    ...
    it('should pass siteId and departmentId to findActiveEmployees', async () => {
      ...
      expect(employeeRepo.findActiveEmployees).toHaveBeenCalledWith('site-1', 'dept-1')
    })

    it('should handle records with null clockIn/clockOut', async () => {
      const employees = [{ id: 'emp-1', fullName: 'John', employeeNumber: 'E001' }]
      const records = [
        { employeeId: 'emp-1', date: new Date('2024-03-01'), status: 'absent', clockIn: null, clockOut: null, overtimeHours: 0 },
      ]
  ...
  describe('recordAttendance', () => {
    it('should create new attendance record when none exists', async () => {
      const command = new RecordAttendanceCommand('emp-1', '2024-03-01', '08:00', '17:00', 'present', 'On time')
    ...
    it('should update existing attendance record', async () => {
      const command = new RecordAttendanceCommand('emp-1', '2024-03-01', '09:00', '18:00', 'late')
      const existing = { id: 'att-1', clockIn: new Date('2024-03-01T08:00:00') }
    ...
    it('should default status to present when not provided', async () => {
      const command = new RecordAttendanceCommand('emp-1', '2024-03-01', '08:00', '17:00')
    ...
    it('should parse ISO time strings for clockIn/clockOut', async () => {
      const command = new RecordAttendanceCommand('emp-1', '2024-03-01', '2024-03-01T08:30:00', '2024-03-01T17:30:00')
    ...
    it('should use existing clockIn/clockOut when command values are undefined on update', async () => {
      ...
      const command = new RecordAttendanceCommand('emp-1', '2024-03-01', undefined, undefined, 'late')
      const existing = { id: 'att-1', clockIn: existingClockIn, clockOut: existingClockOut, absenceReason: 'reason' }
  ...
  describe('importCsv', () => {
    it('should import CSV lines and count flagged late arrivals', async () => {
      const lines = [
        { employeeId: 'emp-1', date: '2024-03-01', clockIn: '2024-03-01T07:50:00', clockOut: '2024-03-01T17:00:00' },
        { employeeId: 'emp-2', date: '2024-03-01', clockIn: '2024-03-01T08:30:00', clockOut: '2024-03-01T17:00:00' },
        { employeeId: 'emp-3', date: '2024-03-01', clockIn: '2024-03-01T08:01:00', clockOut: '2024-03-01T17:00:00' },
      ]
    ...
    it('should update existing records during import', async () => {
      const lines = [
        { employeeId: 'emp-1', date: '2024-03-01', clockIn: '2024-03-01T08:00:00', clockOut: '2024-03-01T17:00:00' },
      ]

      attendanceRepo.findByEmployeeAndDate.mockResolvedValue({ id: 'existing-att' })
      attendanceRepo.update.mockResolvedValue({})
      ...
      expect(attendanceRepo.update).toHaveBeenCalledWith('existing-att', expect.objectContaining({
        isImported: true,
      }))
    })
    ...
    it('should mark late when clockIn is after 08:00', async () => {
      const lines = [
        { employeeId: 'emp-1', date: '2024-03-01', clockIn: '2024-03-01T07:59:00', clockOut: '2024-03-01T17:00:00' },
        { employeeId: 'emp-2', date: '2024-03-01', clockIn: '2024-03-01T08:01:00', clockOut: '2024-03-01T17:00:00' },
      ]
      ...
      expect(attendanceRepo.create).toHaveBeenNthCalledWith(1, expect.objectContaining({ status: 'present' }))
      expect(attendanceRepo.create).toHaveBeenNthCalledWith(2, expect.objectContaining({ status: 'late' }))
    })
  })
