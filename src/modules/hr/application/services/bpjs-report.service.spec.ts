...
describe('BpjsReportService', () => {
  ...
  describe('generateReport', () => {
    it('should generate report with active enrollment and payroll data', async () => {
      ...
      const employee = { id: 'emp-1', fullName: 'John Doe', employeeNumber: 'E001' }
      ...
      payrollRepo.findRunByMonthYear.mockImplementation((month: number, year: number) => {
    ...
    it('should detect new enrollment status', async () => {
      ...
      employeeRepo.findById.mockResolvedValue({ id: 'emp-1', fullName: 'John', employeeNumber: 'E001' })
    ...
    it('should detect salary_changed status when salary differs from prior month', async () => {
      ...
      bpjsRepo.findActiveEnrollments.mockResolvedValue(enrollments)
      payrollRepo.findRunByMonthYear.mockImplementation((month: number, year: number) => {
        if (month === 3 && year === 2024) return Promise.resolve({ id: 'run-current' })
        if (month === 2 && year === 2024) return Promise.resolve({ id: 'run-prior' })
        return Promise.resolve(null)
      })
      payrollRepo.findDetailsByRunId.mockImplementation((runId: string) => {
        if (runId === 'run-current') return Promise.resolve([{ employeeId: 'emp-1', basicSalary: 6000000, bpjsKesehatanEmployee: 60000, bpjsKesehatanEmployer: 240000, bpjsJkk: 14400, bpjsJkm: 18000, bpjsJht: 120000, bpjsJp: 60000 }])
        if (runId === 'run-prior') return Promise.resolve([{ employeeId: 'emp-1', basicSalary: 5000000 }])
    ...
    it('should skip enrollments where employee is not found', async () => {
      const enrollments = [
        { employeeId: 'emp-ghost', program: 'JHT', memberNumber: 'BPJS-999', salary: 5000000, enrollmentDate: '2023-01-01', endDate: null },
      ]
  ...
  describe('exportReport', () => {
    it('should export CSV with BPJS portal format', async () => {
      ...
      payrollRepo.findRunByMonthYear.mockResolvedValue(null)
      payrollRepo.findDetailsByRunId.mockResolvedValue([])
      employeeRepo.findById.mockResolvedValue({ id: 'emp-1', fullName: 'John Doe', employeeNumber: 'E001' })
