...
export class AssetTypeOrmRepository implements AssetRepositoryPort {
  ...
  constructor(private readonly dataSource: DataSource) {
    ...
    this.depreciationRepo = dataSource.getRepository(AssetDepreciationTypeOrmEntity)
  }

  async findAll(filters?: {
    search?: string; category?: string; status?: string;
    depreciationMethod?: string; page?: number; limit?: number;
    ...
    if (filters?.depreciationMethod) {
      qb.andWhere('asset.depreciationMethod = :method', { method: filters.depreciationMethod })
    }
  ...
  async findAssetsDueForDepreciation(schedule: string, beforeDate: Date): Promise<any[]> {
    const qb = this.assetRepo.createQueryBuilder('asset')
    qb.where('asset.status = :status', { status: 'active' })
    qb.andWhere('asset.depreciationSchedule = :schedule', { schedule })
    qb.andWhere('(asset.lastDepreciationDate IS NULL OR asset.lastDepreciationDate < :beforeDate)', { beforeDate })
