...
export interface AssetRepositoryPort {
  findAll(filters?: { search?: string; category?: string; status?: string; depreciationMethod?: string; page?: number; limit?: number }): Promise<{ data: any[]; total: number }>
  ...
  findAssetsDueForDepreciation(schedule: string, beforeDate: Date): Promise<any[]>
}
