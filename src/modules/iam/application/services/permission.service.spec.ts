...
describe('PermissionService', () => {
  ...
  describe('findAll', () => {
    it('should return paginated permissions', async () => {
      const expectedResult = {
        data: [new Permission({ id: '1', name: 'Read Users', resource: 'users', action: 'read' })],
      ...
      expect(mockPermissionRepository.findAll).toHaveBeenCalledWith({ page: 1, limit: 20 });
    });
  ...
  describe('create', () => {
    it('should create a permission', async () => {
      const command = new CreatePermissionCommand('Create User', 'users', 'create');
      const savedPermission = new Permission({ id: 'perm-1', name: 'Create User', resource: 'users', action: 'create' });
