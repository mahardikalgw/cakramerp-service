...
export class SpendingService {
  ...
  async findAll(filters?: {
    search?: string; category?: string; status?: string; startDate?: string; endDate?: string; page?: number; limit?: number;
  }): Promise<{ data: any[]; total: number }> {
    const qb = this.spendingRepo.createQueryBuilder('sp')

    if (filters?.search) {
      qb.andWhere('(sp.description ILIKE :search OR sp.spendingNumber ILIKE :search OR sp.reference ILIKE :search)', { search: `%${filters.search}%` })
    }
    if (filters?.category) {
  ...
  async create(data: {
    ...
    return this.spendingRepo.save(this.spendingRepo.create({
      spendingNumber,
  ...
  async postToGL(id: string): Promise<any> {
    ...
    const accountInfo = CATEGORY_ACCOUNT_MAP[spending.category] ?? CATEGORY_ACCOUNT_MAP['other']!

    const queueEntry = await this.queueRepo.save(this.queueRepo.create({
      sourceType: 'spending',
  ...
  async delete(id: string): Promise<void> {
    ...
    if (spending.glPostingQueueId) {
      await this.queueRepo.update(spending.glPostingQueueId, { status: 'cancelled' } as any)
    }
