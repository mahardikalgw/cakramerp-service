...
export class FinanceService {
  ...
  async getKpiData(): Promise<KpiData> {
    ...
    const currentRevenueLines = await this.journalLineRepo.findByAccountIdsAndDateRange(
      revenueAccountIds,
      startOfMonth,
      now,
    )
    const currentExpenseLines = await this.journalLineRepo.findByAccountIdsAndDateRange(
      expenseAccountIds,
      startOfMonth,
      now,
    )
    ...
    const priorRevenueLines = await this.journalLineRepo.findByAccountIdsAndDateRange(
      revenueAccountIds,
      startOfLastMonth,
      endOfLastMonth,
    )
    const priorExpenseLines = await this.journalLineRepo.findByAccountIdsAndDateRange(
      expenseAccountIds,
      startOfLastMonth,
      endOfLastMonth,
    )
    ...
    const cashLines = cashAccountIds.length > 0
    ...
    const arTotal = outstandingInvoices.reduce((sum, inv) => sum.plus(inv.balance), new Decimal(0))
    const apTotal = outstandingPayments.reduce((sum, p) => sum.plus(p.amount), new Decimal(0))

    const avgCompletion = activeProjects.length > 0
      ? activeProjects.reduce((sum, p) => sum + p.completionPercent, 0) / activeProjects.length
      : 0
    ...
    const expensesChange = priorExpenses.isZero()
      ? 0
      : expensesMtd.minus(priorExpenses).div(priorExpenses).times(100).toNumber()
    ...
    const netProfitChange = priorNetProfit.isZero()
      ? 0
      : netProfit.minus(priorNetProfit).div(priorNetProfit.abs()).times(100).toNumber()
  ...
  async getReceivablesAging(filters?: { clientId?: string; segment?: string; projectId?: string }): Promise<AgingBucket[]> {
    ...
    const byClient = new Map<string, { clientName: string; invoices: typeof filtered }>()

    for (const inv of filtered) {
      if (!byClient.has(inv.clientId)) {
        byClient.set(inv.clientId, { clientName: inv.clientName, invoices: [] })
      }
  ...
  async getCashFlow(): Promise<CashFlowData> {
    ...
    const allInvoices = await this.arInvoiceRepo.findByDateRange(startOfYear, now)
    const allPayments = await this.apPaymentRepo.findByDateRange(startOfYear, now)
    ...
    return {
      inflows: [
        { category: 'Operations', month: operationsInflowMonth, ytd: operationsInflowYtd },
        { category: 'Project Receipts', month: projectReceiptsMonth, ytd: projectReceiptsYtd },
        { category: 'Loan Repayments', month: 0, ytd: 0 },
      ],
      outflows: [
        { category: 'Operations', month: operationsOutflowMonth, ytd: operationsOutflowYtd },
        { category: 'Project Costs', month: projectCostsMonth, ytd: projectCostsYtd },
        { category: 'Loan Payments', month: loanPaymentsMonth, ytd: loanPaymentsYtd },
      ],
    }
  }
  ...
  async getProfitLossReport(filters?: {
    ...
    const currentLines = await this.journalLineRepo.findByDateRange(dateFrom, dateTo, journalFilters)
    // Get prior period lines
    const priorLines = await this.journalLineRepo.findByDateRange(priorFrom, priorTo, journalFilters)
    ...
    const calcAccountTotal = (
      ...
      if (type === 'revenue') {
        return accountLines.reduce((sum, l) => sum + l.credit.minus(l.debit).toNumber(), 0)
      }
      return accountLines.reduce((sum, l) => sum + l.debit.minus(l.credit).toNumber(), 0)
    }
    ...
    const buildLineItems = (
      ...
      return accounts.map((account) => {
        const currentPeriod = calcAccountTotal(currentLines, account.id, type)
        const priorPeriod = calcAccountTotal(priorLines, account.id, type)
        const variance = currentPeriod - priorPeriod
        const variancePercent = priorPeriod !== 0 ? (variance / Math.abs(priorPeriod)) * 100 : 0
        return {
          label: account.name,
          currentPeriod,
          priorPeriod,
    ...
    const sumItems = (items: PlLineItem[]): { current: number; prior: number } => ({
    ...
    const buildSummaryItem = (label: string, current: number, prior: number): PlLineItem => {
      const variance = current - prior
      const variancePercent = prior !== 0 ? (variance / Math.abs(prior)) * 100 : 0
      return { label, currentPeriod: current, priorPeriod: prior, variance, variancePercent: Math.round(variancePercent * 10) / 10 }
    }
    ...
    return {
      ...
      operatingExpenses,
      summary: {
        totalRevenue: buildSummaryItem('Total Revenue', revTotals.current, revTotals.prior),
        totalCogs: buildSummaryItem('Total COGS', cogsTotals.current, cogsTotals.prior),
        grossProfit: buildSummaryItem('Gross Profit', grossProfitCurrent, grossProfitPrior),
        totalOperatingExpenses: buildSummaryItem('Total Operating Expenses', opexTotals.current, opexTotals.prior),
        netProfit: buildSummaryItem('Net Profit', netProfitCurrent, netProfitPrior),
      },
    }
  }

  async getAlertSettings(): Promise<{ alertType: string; value: number; isActive: boolean }[]> {
    const thresholds = await this.thresholdRepo.findAll()
    return thresholds.map((t) => ({ alertType: t.alertType, value: t.value, isActive: t.isActive }))
  }

  async updateAlertSettings(settings: { alertType: string; value: number }[]): Promise<void> {
  ...
  async checkThresholdsAndCreateAlerts(): Promise<void> {
    ...
    for (const threshold of thresholds.filter((t) => t.isActive)) {
      ...
      if (message) {
        ...
        const existing = await this.alertRepo.findByType(threshold.alertType, yesterday)
  ...
  async getStatementOfAccount(clientId: string): Promise<{
    clientId: string
    clientName: string
    invoices: { invoiceNumber: string; issueDate: string; dueDate: string; amount: number; paid: number; balance: number; status: string }[]
    grandTotal: number
  }> {
    ...
    const grandTotal = outstanding.reduce((sum, inv) => sum + inv.balance.toNumber(), 0)
