...
export class BillingLetterService {
  ...
  async findAll(filters?: {
    type?: string; status?: string; customerId?: string; supplierId?: string; page?: number; limit?: number;
  }): Promise<{ data: any[]; total: number }> {
    let where = 'WHERE 1=1'
    const params: any[] = []
    let paramIdx = 1

    if (filters?.type) { where += ` AND bl.type = $${paramIdx++}`; params.push(filters.type) }
    if (filters?.status) { where += ` AND bl.status = $${paramIdx++}`; params.push(filters.status) }
    if (filters?.customerId) { where += ` AND bl.customer_id = $${paramIdx++}`; params.push(filters.customerId) }
    if (filters?.supplierId) { where += ` AND bl.supplier_id = $${paramIdx++}`; params.push(filters.supplierId) }

    const page = filters?.page ?? 1
    const limit = filters?.limit ?? 20
    const offset = (page - 1) * limit

    const countResult = await this.dataSource.query(`SELECT COUNT(*) as total FROM billing_letters bl ${where}`, params)
  ...
  async findById(id: string): Promise<any> {
    const rows = await this.dataSource.query(`SELECT * FROM billing_letters WHERE id = $1`, [id])
  ...
  async generate(data: {
    ...
    const items: any[] = []
    const invoiceTable = data.type === 'receivable' ? 'ar_invoices' : 'ap_invoices'
    const dateField = data.type === 'receivable' ? 'issue_date' : 'invoice_date'

    if (data.type === 'receivable' && data.customerId) {
      const custRows = await this.dataSource.query(`SELECT name FROM customers WHERE id = $1`, [data.customerId])
      partyName = custRows[0]?.name ?? ''
    } else if (data.type === 'payable' && data.supplierId) {
      const supRows = await this.dataSource.query(`SELECT name FROM suppliers WHERE id = $1`, [data.supplierId])
    ...
    for (const invoiceId of data.invoiceIds) {
      ...
      if (invRows.length) {
        ...
        const daysOverdue = inv.due_date ? Math.max(0, Math.floor((today.getTime() - new Date(inv.due_date).getTime()) / (1000 * 60 * 60 * 24))) : 0
        items.push({
          invoiceId: inv.id,
    ...
    if (paymentAmount > totalOutstanding) {
      throw new BadRequestException(`Payment amount (${paymentAmount}) exceeds total outstanding (${totalOutstanding})`)
    }
    ...
    for (const item of items) {
      await this.dataSource.query(
        ...
        [letter.id, item.invoiceId, item.invoiceNumber, item.invoiceDate, item.dueDate, item.amount, item.paidAmount, item.outstandingAmount, item.daysOverdue],
      )
    }
    ...
    if (data.type === 'payable' && data.supplierId) {
      ...
      await queueRepo.save(queueRepo.create({
        ...
        suggestedLines: [
          { accountId: '', accountCode: '2100', accountName: 'Accounts Payable', debit: paymentAmount, credit: 0, description: `Pay ${letterNumber}` },
          { accountId: '', accountCode: '1100', accountName: 'Bank / Cash', debit: 0, credit: paymentAmount, description: `Payment to ${partyName}` },
        ],
        status: 'pending',
      ...
      await queueRepo.save(queueRepo.create({
        ...
        suggestedLines: [
          { accountId: '', accountCode: '1100', accountName: 'Bank / Cash', debit: paymentAmount, credit: 0, description: `Receipt from ${partyName}` },
          { accountId: '', accountCode: '1200', accountName: 'Accounts Receivable', debit: 0, credit: paymentAmount, description: `Receive ${letterNumber}` },
        ],
        status: 'pending',
   ...
   */
  async applyPayment(billingLetterId: string, journalEntryId: string): Promise<void> {
    ...
    const invoiceTable = letter.type === 'receivable' ? 'ar_invoices' : 'ap_invoices'
    ...
    for (const item of items) {
      ...
      const newStatus = newPaidAmount >= invAmount ? 'paid' : (letter.type === 'receivable' ? 'partially_paid' : 'pending')
    ...
    const status = totalPaidApplied >= Number(letter.total_amount) ? 'paid' : 'partially_paid'
    await this.dataSource.query(
  ...
  private async generateLetterNumber(type: string): Promise<string> {
    ...
    const lastNum = parseInt(result[0].letter_number.replace(prefix, ''), 10) + 1
  ...
  async delete(id: string): Promise<void> {
    ...
    if (letter.status === 'paid' || letter.status === 'partially_paid') {
      throw new BadRequestException('Cannot delete a billing letter that has been paid or partially paid')
    }
    if (Number(letter.paid_amount) > 0) {
      throw new BadRequestException('Cannot delete a billing letter with payments recorded')
    }
    ...
    if (postedQueue) {
      throw new BadRequestException('Cannot delete: GL Posting Queue entry has already been posted to journal')
    }
    ...
    await this.dataSource.query(
      `DELETE FROM billing_letters WHERE id = $1`,
      [id],
    )
  }
}
