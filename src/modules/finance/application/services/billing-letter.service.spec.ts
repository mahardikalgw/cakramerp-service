...
describe('BillingLetterService', () => {
  ...
  describe('findAll', () => {
    it('should return billing letters with filters', async () => {
      mockDataSource.query
        ...
        .mockResolvedValueOnce([
          { id: '1', letter_number: 'BL-AR-2024-0001', type: 'receivable', status: 'outstanding' },
          { id: '2', letter_number: 'BL-AR-2024-0002', type: 'receivable', status: 'paid' },
        ]);

      const result = await service.findAll({ type: 'receivable', page: 1, limit: 20 });
    ...
    it('should apply all filter parameters', async () => {
      ...
      await service.findAll({ type: 'payable', status: 'outstanding', customerId: 'c1', supplierId: 's1', page: 2, limit: 5 });
  ...
  describe('findById', () => {
    it('should return billing letter with items', async () => {
      mockDataSource.query
        .mockResolvedValueOnce([{ id: '1', letter_number: 'BL-AR-2024-0001', type: 'receivable' }])
        .mockResolvedValueOnce([
          { id: 'i1', billing_letter_id: '1', invoice_number: 'INV-001', outstanding_amount: 500 },
        ]);
  ...
  describe('applyPayment', () => {
    it('should apply payment FIFO to invoices for payable', async () => {
      ...
      // Items query
      mockDataSource.query.mockResolvedValueOnce([
        { id: 'i1', invoice_id: 'inv1', invoice_number: 'INV-001', due_date: '2024-01-15' },
        { id: 'i2', invoice_id: 'inv2', invoice_number: 'INV-002', due_date: '2024-01-31' },
      ]);
      // Invoice 1 query
      mockDataSource.query.mockResolvedValueOnce([{ amount: 300, paid_amount: 0, status: 'pending' }]);
      // Update invoice 1
      mockDataSource.query.mockResolvedValueOnce(undefined);
      // Update billing letter item 1
      mockDataSource.query.mockResolvedValueOnce(undefined);
      // Invoice 2 query
      mockDataSource.query.mockResolvedValueOnce([{ amount: 400, paid_amount: 0, status: 'pending' }]);
      // Update invoice 2
      mockDataSource.query.mockResolvedValueOnce(undefined);
      ...
      expect(mockSubsidiaryLedgerService.recordApEntry).toHaveBeenCalledTimes(2);
      expect(mockSubsidiaryLedgerService.recordApEntry).toHaveBeenCalledWith(expect.objectContaining({
        debit: 300,
        invoiceId: 'inv1',
      }));
      expect(mockSubsidiaryLedgerService.recordApEntry).toHaveBeenCalledWith(expect.objectContaining({
    ...
    it('should apply payment for receivable type', async () => {
      ...
      mockDataSource.query.mockResolvedValueOnce([{ amount: 500, paid_amount: 0, status: 'outstanding' }]);
      ...
      expect(mockSubsidiaryLedgerService.recordArEntry).toHaveBeenCalledWith(expect.objectContaining({
        credit: 500,
        customerId: 'c1',
      }));
    });
  ...
  describe('updateStatus', () => {
    it('should recompute billing letter status as paid', async () => {
      ...
      mockDataSource.query.mockResolvedValueOnce([{ id: 'bl1', type: 'receivable', status: 'outstanding', paid_amount: 0 }]);
      // findById: items query
      mockDataSource.query.mockResolvedValueOnce([{ id: 'i1', invoice_id: 'inv1' }]);
      // Invoice query
      mockDataSource.query.mockResolvedValueOnce([{ amount: 1000, paid_amount: 1000 }]);
      // Update item
      mockDataSource.query.mockResolvedValueOnce(undefined);
      // Update letter
      mockDataSource.query.mockResolvedValueOnce(undefined);
      // Final findById: letter query
      mockDataSource.query.mockResolvedValueOnce([{ id: 'bl1', type: 'receivable', status: 'paid', paid_amount: 1000 }]);
      // Final findById: items query
      mockDataSource.query.mockResolvedValueOnce([{ id: 'i1', invoice_id: 'inv1', paid_amount: 1000, outstanding_amount: 0 }]);
    ...
    it('should recompute status as partially_paid', async () => {
      ...
      mockDataSource.query.mockResolvedValueOnce([{ id: 'i1', invoice_id: 'inv1' }]);
      // Invoice query
      mockDataSource.query.mockResolvedValueOnce([{ amount: 1000, paid_amount: 500 }]);
      // Update item
      mockDataSource.query.mockResolvedValueOnce(undefined);
      // Update letter
      mockDataSource.query.mockResolvedValueOnce(undefined);
      // Final findById: letter query
      mockDataSource.query.mockResolvedValueOnce([{ id: 'bl1', type: 'receivable', status: 'partially_paid', paid_amount: 500 }]);
      // Final findById: items query
      mockDataSource.query.mockResolvedValueOnce([{ id: 'i1', invoice_id: 'inv1', paid_amount: 500, outstanding_amount: 500 }]);
  ...
  describe('delete', () => {
    it('should delete an outstanding billing letter', async () => {
      // findById: letter query
      mockDataSource.query.mockResolvedValueOnce([{ id: 'bl1', status: 'outstanding', paid_amount: 0 }]);
      // findById: items query
      mockDataSource.query.mockResolvedValueOnce([]);
      // GL queue check
      mockDataSource.query.mockResolvedValueOnce([]);
    ...
    it('should throw if letter is paid', async () => {
      // findById: letter query
      mockDataSource.query.mockResolvedValueOnce([{ id: 'bl1', status: 'paid', paid_amount: 1000 }]);
      // findById: items query
      mockDataSource.query.mockResolvedValueOnce([]);
    ...
    it('should throw if letter has payments recorded', async () => {
      // findById: letter query
      mockDataSource.query.mockResolvedValueOnce([{ id: 'bl1', status: 'outstanding', paid_amount: 500 }]);
      // findById: items query
      mockDataSource.query.mockResolvedValueOnce([]);
    ...
    it('should throw if GL queue entry has been posted', async () => {
      ...
      mockDataSource.query.mockResolvedValueOnce([]);
      // GL queue check
      mockDataSource.query.mockResolvedValueOnce([{ id: 'q1', status: 'posted' }]);
