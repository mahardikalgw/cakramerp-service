...
export class FinancialStatementsService implements FinancialStatementsServicePort {
  ...
  async getProfitLoss(
    ...
    const calcAmount = (
      ...
      if (type === 'revenue') {
        return accountLines.reduce(
          (sum: number, l: any) =>
            sum + l.credit.toNumber() - l.debit.toNumber(),
          0,
        );
      }
      return accountLines.reduce(
        (sum: number, l: any) => sum + l.debit.toNumber() - l.credit.toNumber(),
        0,
      );
    };
    ...
    const buildItems = (
      ...
      return accounts
        .map((acc) => ({
          ...
          priorAmount: comparePrior
            ? calcAmount(priorLines, acc.id, type)
            : undefined,
        }))
        .filter(
          (item) =>
            item.amount !== 0 || (item.priorAmount && item.priorAmount !== 0),
        );
    };
    ...
    const priorTotalRevenue = comparePrior
      ? revenue.reduce((sum, i) => sum + (i.priorAmount ?? 0), 0)
      : undefined;
    const priorTotalCogs = comparePrior
      ? cogs.reduce((sum, i) => sum + (i.priorAmount ?? 0), 0)
      : undefined;
    const priorTotalOperatingExpenses = comparePrior
  ...
  async getBalanceSheet(asOfDate: string): Promise<BalanceSheetStatement> {
    ...
    const assets: StatementLineItem[] = assetAccounts
      .map((acc) => ({
        code: acc.code,
        name: acc.name,
        amount: calcBalance(acc.id, 'asset'),
      }))
      .filter((i) => i.amount !== 0);
    ...
    const liabilities: StatementLineItem[] = liabilityAccounts
      .map((acc) => ({
        code: acc.code,
        name: acc.name,
        amount: calcBalance(acc.id, 'liability'),
      }))
      .filter((i) => i.amount !== 0);
    ...
    const equity: StatementLineItem[] = equityAccounts
      .map((acc) => ({
        code: acc.code,
        name: acc.name,
        amount: calcBalance(acc.id, 'equity'),
      }))
      .filter((i) => i.amount !== 0);
  ...
  async getCashFlow(
    ...
    for (const [desc, amount] of grouped) {
      ...
      if (
        desc.toLowerCase().includes('loan') ||
        desc.toLowerCase().includes('equity')
      ) {
        financing.push(item);
      } else if (
        desc.toLowerCase().includes('invest') ||
        desc.toLowerCase().includes('asset')
      ) {
        investing.push(item);
      } else {
        operating.push(item);
  ...
  async exportCsv(
    ...
    if (type === 'profit-loss') {
      ...
      for (const item of data.revenue) {
        rows.push(
          `${item.code},"${item.name}",${item.amount},${item.priorAmount ?? 0}`,
        );
      }
      rows.push(
        `,"Total Revenue",${data.totalRevenue},${data.priorTotalRevenue ?? 0}`,
      );
      rows.push('--- COGS ---,,,');
      ...
      rows.push(
        `,"Gross Profit",${data.grossProfit},${data.priorGrossProfit ?? 0}`,
      );
      ...
      rows.push(
        `,"Total Operating Expenses",${data.totalOperatingExpenses},${data.priorTotalOperatingExpenses ?? 0}`,
      );
