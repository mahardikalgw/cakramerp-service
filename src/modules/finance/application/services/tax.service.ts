...
export class TaxService {
  ...
  async getMonthlyReport(month: number, year: number): Promise<TaxReportSummary> {
    ...
    const invoiceResponses: TaxInvoiceResponse[] = invoices.map((inv: TaxInvoiceTypeOrmEntity) => {
      ...
      return {
        ...
        transactionDate: inv.transactionDate?.toISOString?.() ?? String(inv.transactionDate),
  ...
  async exportCsv(month: number, year: number): Promise<string> {
    ...
    // FK header row
    rows.push('FK,KD_JENIS_TRANSAKSI,FG_PENGGANTI,NOMOR_FAKTUR,MASA_PAJAK,TAHUN_PAJAK,TANGGAL_FAKTUR,NPWP,NAMA,ALAMAT_LENGKAP,JUMLAH_DPP,JUMLAH_PPN,JUMLAH_PPNBM,ID_KETERANGAN_TAMBAHAN,FG_UANG_MUKA,UANG_MUKA_DPP,UANG_MUKA_PPN,UANG_MUKA_PPNBM,REFERENSI')
  ...
  async exportPdf(month: number, year: number): Promise<string> {
    ...
    const rows: string[] = []
    rows.push(`LAPORAN PPN - Periode: ${String(month).padStart(2, '0')}/${year}`)
  ...
  private validateTaxInvoice(inv: TaxInvoiceTypeOrmEntity): string[] {
    ...
    if (!/^\d{15,16}$/.test(npwpClean)) {
      errors.push(`Invalid NPWP format: "${inv.clientNpwp}" (must be 15-16 digits)`)
    }
    ...
    if (!/^\d{13,16}$/.test(taxNumClean)) {
      errors.push(`Invalid tax invoice number format: "${inv.taxInvoiceNumber}"`)
    }
    ...
    if (Math.abs(actualPpn - expectedPpn) > 1) {
      errors.push(`PPN amount (${actualPpn}) doesn't match 11% of DPP (expected ~${Math.round(expectedPpn)})`)
    }
