...
export class BankReconciliationService implements BankReconciliationServicePort {
  ...
  async importStatement(
    ...
    userId: string,
  ): Promise<{ sessionId: string; linesImported: number }> {
  ...
  async autoMatch(sessionId: string): Promise<{ matchedCount: number }> {
    ...
    for (const bankLine of bankLines) {
      ...
      const match = glLines.find((gl) => {
        ...
        const dayDiff =
          Math.abs(bankDate.getTime() - glDate.getTime()) /
          (1000 * 60 * 60 * 24);
        return dayDiff <= 1;
      });
  ...
  async manualMatch(
    sessionId: string,
    command: ManualReconciliationMatchCommand,
  ): Promise<void> {
    const session = await this.sessionRepo.findOne({
      where: { id: sessionId },
    });
    ...
    const bankLine = await this.statementLineRepo.findOne({
      where: {
        id: command.bankStatementLineId,
        reconciliationSessionId: sessionId,
      },
    });
    if (!bankLine)
      throw new BadRequestException('Bank statement line not found');
  ...
  async getReport(sessionId: string): Promise<ReconciliationReport> {
    ...
    const matchedBankLines = allBankLines.filter(
      (l: any) => l.matchStatus === 'matched',
    );
    const unmatchedBankLines = allBankLines.filter(
      (l: any) => l.matchStatus === 'unmatched',
    );
    ...
    const matchedGlIds = new Set(
      matchedBankLines.map((l: any) => l.matchedJournalLineId),
    );
    const unmatchedGlLines = glLines.filter((l) => !matchedGlIds.has(l.id));

    const matchedTotal = matchedBankLines.reduce(
      (sum: number, l: any) =>
        sum + Math.abs(Number(l.debit) - Number(l.credit)),
      0,
    );

    const bankTotal = allBankLines.reduce(
      (sum: number, l: any) => sum + Number(l.debit) - Number(l.credit),
      0,
    );
    ...
    return {
      ...
      periodStart:
        session.periodStart?.toISOString?.() ?? String(session.periodStart),
      periodEnd:
        session.periodEnd?.toISOString?.() ?? String(session.periodEnd),
