...
export class GlPostingQueueTypeOrmRepository implements GlPostingQueueRepositoryPort {
  ...
  async findAll(filters?: {
    ...
    if (filters?.sourceType) {
      qb.andWhere('pq.sourceType = :sourceType', { sourceType: filters.sourceType })
    }
  ...
  async findBySource(sourceType: string, sourceId: string): Promise<GlPostingQueue | null> {
  ...
  async save(queue: GlPostingQueue): Promise<GlPostingQueue> {
    const entity = this.repo.create({
      ...queue,
      suggestedLines: queue.suggestedLines as any,
  ...
  private toDomain(entity: GlPostingQueueTypeOrmEntity): GlPostingQueue {
    return new GlPostingQueue({
      ...
      suggestedLines: (entity.suggestedLines ?? []) as Record<string, unknown>[],
