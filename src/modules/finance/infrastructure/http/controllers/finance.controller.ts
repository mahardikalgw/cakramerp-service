...
export class FinanceController {
  ...
  async exportProfitLoss(
    ...
    const rows: string[] = [];
    rows.push(
      'Line Item,Current Period,Prior Period,Variance (IDR),Variance (%)',
    );
    rows.push('');
    rows.push('--- Revenue ---,,,,');
    for (const item of data.revenue) {
      rows.push(
        `"${item.label}",${item.currentPeriod},${item.priorPeriod},${item.variance},${item.variancePercent}%`,
      );
    }
    rows.push(
      `"${data.summary.totalRevenue.label}",${data.summary.totalRevenue.currentPeriod},${data.summary.totalRevenue.priorPeriod},${data.summary.totalRevenue.variance},${data.summary.totalRevenue.variancePercent}%`,
    );
    rows.push('');
    ...
    rows.push(
      `"${data.summary.totalCogs.label}",${data.summary.totalCogs.currentPeriod},${data.summary.totalCogs.priorPeriod},${data.summary.totalCogs.variance},${data.summary.totalCogs.variancePercent}%`,
    );
    rows.push('');
    rows.push(
      `"${data.summary.grossProfit.label}",${data.summary.grossProfit.currentPeriod},${data.summary.grossProfit.priorPeriod},${data.summary.grossProfit.variance},${data.summary.grossProfit.variancePercent}%`,
    );
    rows.push('');
    ...
    rows.push(
      `"${data.summary.totalOperatingExpenses.label}",${data.summary.totalOperatingExpenses.currentPeriod},${data.summary.totalOperatingExpenses.priorPeriod},${data.summary.totalOperatingExpenses.variance},${data.summary.totalOperatingExpenses.variancePercent}%`,
    );
    rows.push('');
    rows.push(
      `"${data.summary.netProfit.label}",${data.summary.netProfit.currentPeriod},${data.summary.netProfit.priorPeriod},${data.summary.netProfit.variance},${data.summary.netProfit.variancePercent}%`,
    );
  ...
  async getStatementOfAccount(
    ...
    for (const inv of data.invoices) {
      rows.push(
        `"${inv.invoiceNumber}",${inv.issueDate},${inv.dueDate},${inv.amount},${inv.paid},${inv.balance},${inv.status}`,
      );
    }
    rows.push('');
