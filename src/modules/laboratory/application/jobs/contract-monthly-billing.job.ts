...
export class ContractMonthlyBillingJob {
  ...
  async generateMonthlyInvoices() {
    this.logger.log('[CRON] Generating monthly invoices for active unlimited contracts...');
    try {
      const result = await this.contractRepo.findAll({ filters: { status: 'active' } });
      const activeContracts = result.data.filter(c => (c as any).isUnlimited);

      for (const contract of activeContracts) {
        try {
          const billingStart = (contract as any).lastBillingDate || (contract as any).billingStartDate;
          if (!billingStart) {
            this.logger.warn(`[CRON] Contract ${contract.id} has no billing start date, skipping`);
            continue;
          }
          ...
          const existing = await this.invoiceRepo.findByBillingPeriod(contract.id, new Date(billingStart), billingEnd);
          if (existing) {
            this.logger.log(`[CRON] Invoice already exists for contract ${contract.id}, skipping`);
            continue;
          }

          // Count confirmed testing results in period
          const results = await this.testResultRepo.findCompletedByContractAndPeriod(
            contract.id,
          ...
          if (results.length === 0) {
            this.logger.log(`[CRON] No completed tests for contract ${contract.id} in this period, skipping`);
            continue;
          }
          ...
          this.logger.log(`[CRON] Generated invoice ${invoiceNumber} for contract ${contract.id}`);
        } catch (err: any) {
          this.logger.error(`[CRON] Failed to generate invoice for contract ${contract.id}: ${err?.message}`);
        }
      }
    } catch (err: any) {
      this.logger.error(`[CRON] Monthly billing job failed: ${err?.message}`, err?.stack);
    }
  }
}