...
@Injectable()
export class StockIssuanceTypeOrmRepository
  implements StockIssuanceRepositoryPort
{
  ...
  async createLine(line: Partial<StockIssuanceLine>): Promise<StockIssuanceLine> {
  ...
  async update(id: string, data: Partial<StockIssuance>): Promise<StockIssuance> {
    const issuance = await this.issuanceRepo.findOne({ where: { id } })
  ...
  private mapToStockIssuance(entity: StockIssuanceTypeOrmEntity): StockIssuance {
