...
export class EquipmentMaintenanceAlertJob {
  ...
  async handleMaintenanceAlerts(): Promise<void> {
    ...
    for (const schedule of schedules) {
      const equipment = await this.equipmentRepo.findOne({ where: { id: schedule.equipmentId } })
    ...
    this.logger.log(`Maintenance alert check complete. ${schedules.length} alert(s) found.`)
  }
}
