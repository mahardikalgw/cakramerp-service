...
describe('StockOpnameService', () => {
  ...
  describe('create', () => {
    it('should create session and lines from warehouse balances', async () => {
      ...
      expect(stockOpnameRepo.getBalancesForWarehouse).toHaveBeenCalledWith(
        'wh-1',
      );
  ...
  describe('updateCounts', () => {
    it('should update actual quantities and compute variance', async () => {
      ...
      stockOpnameRepo.findLineBySessionAndItem.mockResolvedValue({
        ...mockLine,
      });
    ...
    it('should throw BadRequestException when session is not draft', async () => {
      ...
      await expect(
        service.updateCounts('session-1', [
          { itemId: 'item-1', actualQty: 10 },
        ]),
      ).rejects.toThrow(BadRequestException);
    });
  ...
  describe('submit', () => {
    ...
    it('should throw BadRequestException when session not found', async () => {
      ...
      await expect(service.submit('missing')).rejects.toThrow(
        BadRequestException,
      );
    });
  ...
  describe('approve', () => {
    it('should apply adjustments for lines with variance and update session', async () => {
      ...
      expect(stockMovementService.recordMovement).toHaveBeenNthCalledWith(
        1,
    ...
    it('should throw BadRequestException when session not found', async () => {
      ...
      await expect(service.approve('missing', 'user-1')).rejects.toThrow(
        BadRequestException,
      );
    });
    ...
    it('should throw BadRequestException when session is not pending_approval', async () => {
      ...
      await expect(service.approve('session-1', 'user-1')).rejects.toThrow(
        BadRequestException,
      );
    });
  });
  ...
  describe('findAll', () => {
    it('should return data and total', async () => {
      stockOpnameRepo.findAll.mockResolvedValue({
        data: [mockSession],
        total: 1,
      });
      ...
      expect(stockOpnameRepo.findAll).toHaveBeenCalledWith({
        warehouseId: 'wh-1',
        status: 'draft',
      });
