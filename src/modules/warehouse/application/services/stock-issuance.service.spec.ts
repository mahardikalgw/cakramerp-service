...
import { StockIssuanceService, CreateStockIssuanceDto } from './stock-issuance.service'
...
describe('StockIssuanceService', () => {
  ...
  describe('create', () => {
    ...
    it('should create issuance and record negative stock movements', async () => {
      ...
      expect(stockMovementService.getStockBalance).toHaveBeenCalledWith('item-1', 'wh-1')
      expect(stockIssuanceRepo.create).toHaveBeenCalledWith(expect.objectContaining({
        issuanceNumber: `ISS-${year}-0001`,
        status: 'confirmed',
        createdBy: 'user-1',
      }))
      expect(stockMovementService.recordMovement).toHaveBeenCalledWith(expect.objectContaining({
    ...
    it('should throw BadRequestException when stock is insufficient', async () => {
      ...
      await expect(service.create(validDto, 'user-1')).rejects.toThrow(/Insufficient stock/)
    ...
    it('should increment issuance number from last number', async () => {
      ...
      stockIssuanceRepo.getLastIssuanceNumber.mockResolvedValue(`ISS-${year}-0010`)
      stockIssuanceRepo.create.mockResolvedValue(mockIssuance)
      stockIssuanceRepo.createLine.mockResolvedValue(mockIssuanceLine)
      stockMovementService.recordMovement.mockResolvedValue({})
    ...
    it('should process multiple lines', async () => {
      ...
      stockIssuanceRepo.createLine
        ...
        .mockResolvedValueOnce({ ...mockIssuanceLine, id: 'iss-line-2', itemId: 'item-2' })
      stockMovementService.recordMovement.mockResolvedValue({})
  ...
  describe('reverse', () => {
    it('should reverse an issuance and create positive movements', async () => {
      ...
      stockMovementService.recordMovement.mockResolvedValue({})
      stockIssuanceRepo.update.mockResolvedValue({ ...mockIssuance, status: 'reversed' })
      ...
      expect(stockIssuanceRepo.update).toHaveBeenCalledWith('iss-1', expect.objectContaining({
        status: 'reversed',
    ...
    it('should throw BadRequestException when issuance not found', async () => {
      ...
      await expect(service.reverse('missing', 'reason', 'user-1')).rejects.toThrow(BadRequestException)
    })
    ...
    it('should throw BadRequestException when issuance is already reversed', async () => {
      ...
      await expect(service.reverse('iss-1', 'reason', 'user-1')).rejects.toThrow(BadRequestException)
      await expect(service.reverse('iss-1', 'reason', 'user-1')).rejects.toThrow(/already reversed/)
    })
  })

  describe('findAll', () => {
    it('should return data and total', async () => {
      stockIssuanceRepo.findAll.mockResolvedValue({ data: [mockIssuance], total: 1 })

      const result = await service.findAll({ warehouseId: 'wh-1' })

      expect(stockIssuanceRepo.findAll).toHaveBeenCalledWith({ warehouseId: 'wh-1' })
