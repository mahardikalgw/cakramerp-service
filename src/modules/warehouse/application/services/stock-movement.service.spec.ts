...
import { StockMovementService, RecordMovementDto } from './stock-movement.service'
...
describe('StockMovementService', () => {
  ...
  describe('recordMovement', () => {
    it('should compute new balance, upsert, and create ledger entry', async () => {
      ...
      repo.createLedgerEntry.mockResolvedValue({ id: 'ledger-1', balanceAfter: 150 })
    ...
    it('should handle negative quantity (issuance)', async () => {
      ...
      repo.getBalance.mockResolvedValue(100)
      repo.upsertBalance.mockResolvedValue(undefined)
      repo.createLedgerEntry.mockResolvedValue({ id: 'ledger-2', balanceAfter: 70 })

      const result = await service.recordMovement(dto)

      expect(repo.upsertBalance).toHaveBeenCalledWith('item-1', 'wh-1', -30)
      expect(repo.createLedgerEntry).toHaveBeenCalledWith(expect.objectContaining({ balanceAfter: 70 }))
    })
    ...
    it('should handle zero current balance', async () => {
      ...
      repo.upsertBalance.mockResolvedValue(undefined)
      repo.createLedgerEntry.mockResolvedValue({ id: 'ledger-3', balanceAfter: 10 })
