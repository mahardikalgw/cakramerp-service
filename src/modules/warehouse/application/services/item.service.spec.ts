...
describe('ItemService', () => {
  ...
  beforeEach(async () => {
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [ItemService, { provide: ITEM_REPOSITORY, useValue: repo }],
    }).compile();
  ...
  describe('create', () => {
    ...
    it('should default minStockLevel to 0 when not provided', async () => {
      ...
      expect(repo.create).toHaveBeenCalledWith(
        expect.objectContaining({ minStockLevel: 0 }),
      );
    });
  ...
  describe('update', () => {
    ...
    it('should throw NotFoundException when entity not found', async () => {
      ...
      await expect(
        service.update('missing', new UpdateItemCommand()),
    ...
    it('should throw ConflictException when updating code to an existing one', async () => {
      ...
      await expect(service.update('item-1', command)).rejects.toThrow(
        ConflictException,
      );
    });
