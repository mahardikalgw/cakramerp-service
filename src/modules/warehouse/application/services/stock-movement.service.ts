...
export class StockMovementService implements StockMovementServicePort {
  ...
  async recordMovement(dto: RecordMovementDto): Promise<any> {
    // Get current balance
    const currentBalance = await this.stockMovementRepo.getBalance(dto.itemId, dto.warehouseId)
    const newBalance = currentBalance + dto.quantity

    // Update balance
    await this.stockMovementRepo.upsertBalance(dto.itemId, dto.warehouseId, dto.quantity)
