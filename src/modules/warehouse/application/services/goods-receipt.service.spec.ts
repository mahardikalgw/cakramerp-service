...
describe('GoodsReceiptService', () => {
  ...
  describe('create', () => {
    const validDto: CreateGoodsReceiptDto = {
      ...
      lines: [
        { itemId: 'item-1', itemName: 'Widget', poQty: 100, receivedQty: 95, uom: 'pcs' },
        { itemId: 'item-2', itemName: 'Gear', poQty: 50, receivedQty: 50, uom: 'pcs' },
      ],
    }
    ...
    it('should create receipt, lines, and record stock movements', async () => {
      ...
      goodsReceiptRepo.createLine
        ...
        .mockResolvedValueOnce({ ...mockLine, id: 'grn-line-2', itemId: 'item-2' })
      stockMovementService.recordMovement.mockResolvedValue({})

      const result = await service.create(validDto, 'user-1')

      expect(goodsReceiptRepo.getLastGrnNumber).toHaveBeenCalledWith(expect.stringContaining(`GRN-${year}-`))
      expect(goodsReceiptRepo.create).toHaveBeenCalledWith(expect.objectContaining({
    ...
    it('should compute discrepancyQty correctly', async () => {
      ...
      expect(goodsReceiptRepo.createLine).toHaveBeenNthCalledWith(1, expect.objectContaining({
        discrepancyQty: 5,
      }))
    })
    ...
    it('should throw BadRequestException when lines is undefined', async () => {
      const dto = { warehouseId: 'wh-1', vendorName: 'X', receivedDate: '2025-01-01' } as any
  ...
  describe('findAll', () => {
    it('should return data and total', async () => {
      goodsReceiptRepo.findAll.mockResolvedValue({ data: [mockReceipt], total: 1 })

      const result = await service.findAll({ warehouseId: 'wh-1' })

      expect(goodsReceiptRepo.findAll).toHaveBeenCalledWith({ warehouseId: 'wh-1' })
