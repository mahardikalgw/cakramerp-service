...
export class SeedOvertimeAccounts20250601000003 implements MigrationInterface {
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(`DELETE FROM role_permissions WHERE permission_id IN (SELECT id FROM permissions WHERE name IN ('leave:approve', 'attendance:approve'));`)
    await queryRunner.query(`DELETE FROM permissions WHERE name IN ('leave:approve', 'attendance:approve');`)
    await queryRunner.query(`DELETE FROM accounts WHERE code IN ('5103', '2320');`)
  }
}
